...
function subtitle(image){
	$a = 0;
	while(imagens[$a]){
		if(image==url+imagens[$a].imagem){
			count = imagens[$a].count;
			break;
		}
		$a++;
	}
	$('.bullet').removeClass('active');
	$('.bullet[rel="'+$a+'"]').addClass('active');
	$('#banner-index h1').html(imagens[count].titulo);
	$('#banner-index h2').html(imagens[count].subtitulo);
	$('#banner-index p').html(imagens[count].descricao);
	$('#banner-index ul').html(imagens[count].topicos);
	$('#banner-index a').attr('href',imagens[count].link);
}
	...
	i.prototype = {
		...
		show: function(t) {
			...
			return clearInterval(n.interval), n.$img = e("<img />").css(r.img).bind("load", function(t) {
				...
				subtitle(n.$img[0].src);
				
				//sessvars.imagemSession = n.$img[0].src;
